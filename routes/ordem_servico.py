...
def buscar(id_os: int, db: Session = Depends(get_db), _: Usuario = Depends(get_current_user)):
...
def gerar_preventivas(db: Session = Depends(get_db), _: Usuario = Depends(require_admin)):
...
def criar(dados: OrdemServicoCreate, db: Session = Depends(get_db), _: Usuario = Depends(get_current_user)):
...
def atualizar(id_os: int, dados: OrdemServicoUpdate, db: Session = Depends(get_db), _: Usuario = Depends(get_current_user)):
    os_ = db.get(OrdemServico, id_os)
    if not os_:
        raise HTTPException(status_code=404, detail="Ordem de serviço não encontrada")
